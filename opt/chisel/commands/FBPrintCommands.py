...
class FBHideView(fb.FBCommand):
    def name(self):
        return 'phideview'

    def description(self):
        return 'An incredibly contrived command that hides the current view.'

    def run(self, arguments, options):
        hideview = arguments[0]
        lldb.debugger.HandleCommand('po [{} setHidden:YES]'.format(hideview))
        lldb.debugger.HandleCommand("caflush")

class FBShowView(fb.FBCommand):
    def name(self):
        return 'pshowview'

    def description(self):
        return 'An incredibly contrived command that shows the current view.'

    def run(self, arguments, options):
        showview = arguments[0]
        lldb.debugger.HandleCommand('po [{} setHidden:NO]'.format(showview))
        lldb.debugger.HandleCommand("caflush")
