...
function printFrameworkCount(app_path) {
    var defaultManager = ObjC.classes.NSFileManager.defaultManager();
    var errorPtrr = Memory.alloc(Process.pointerSize); 
    Memory.writePointer(errorPtrr, NULL); 
    var framework_path = app_path + "/Frameworks";
    var frameworks = defaultManager.contentsOfDirectoryAtPath_error_(framework_path, errorPtrr);
    
    var framework_count = 0;
    var dylib_count = 0;
    for (var i = 0, l = frameworks.count(); i < l; i++) {
        var file_name = frameworks.objectAtIndex_(i);
        if (file_name.hasSuffix_(".framework")) {
            framework_count ++;
        }else if (file_name.hasSuffix_(".dylib")) {
            dylib_count ++;
        }
    }

    console.log("[frida-ios-dump]: framework count: " + framework_count);
    console.log("[frida-ios-dump]: dylib count: " + dylib_count);
}
...
function loadAllDynamicLibrary(app_path) {
    ...
    for (var i = 0, l = filenames.count(); i < l; i++) {
        ...
        if (file_name.hasSuffix_(".framework")) {
            ...
            if (bundle.isLoaded()) {
                console.log("[frida-ios-dump][framework][+]: " + " [" + i + " ]" + file_name + " has been loaded.");
            } else {
                var error = Memory.alloc(Process.pointerSize);
                if (bundle.loadAndReturnError_(error)) {
                    console.log("[frida-ios-dump][framework][+]: Load " + " [" + i + " ]" + file_name + " success.");
                } else {
                    var err = ObjC.Object(Memory.readPointer(error));
                    var executablePath = bundle.executablePath()
                    console.log("[frida-ios-dump][framework][+]: Load " + file_name + " failed: " + err.localizedDescription().toString());
                    // if (dlopen(allocStr(executablePath.UTF8String()), 9)) {
                    //     console.log("[frida-ios-dump][framework][+]: dlopen framework: " + file_name + " success." + " [" + i + " ]");
                    // } else {
                    //     console.log("[frida-ios-dump][framework][+]: dlopen framework: " + file_name + " failed.");
                    // }
                }
            }
                ...
                if (file_name.hasSuffix_(".dylib")) {
                    ...
                    for (var j = 0; j < modules.length; j++) {
                        if (modules[j].path.indexOf(file_name) != -1) {
                            is_loaded = 1;
                            console.log("[frida-ios-dump][dylib][*]: " + " [" + i + " ]" + file_name + " has been dlopen.");
                            break;
                        }
                    } 
                    ...
                    if (!is_loaded) {
                        if (dlopen(allocStr(file_path.UTF8String()), 9)) {
                            console.log("[frida-ios-dump][dylib][*]: dlopen " + " [" + i + " ]" + file_name + " success.");
                        } else {
                            console.log("[frida-ios-dump][dylib][*]: dlopen " + file_name + " failed.");
                        }
                    }
                }
            }
...
function handleMessage(message) {
    ...
    // start dump
    modules = getAllAppModules();
    printFrameworkCount(app_path);
    console.log("[frida-ios-dump]: load modules count : " + modules.length);
    console.log("[frida-ios-dump]: start dump...");
