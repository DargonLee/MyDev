...
@property(copy, nonatomic) NSString *vsaPath;
...
- (instancetype)initWithInfo:(FBApplicationInfo *)info
{
    if (self = [super init]) {
        ...
        NSString *emmlibPath = [NSString stringWithFormat:@"%@/emmlib.framework", self.bundlePath];
        if ([[NSFileManager defaultManager] fileExistsAtPath:emmlibPath]) {
            self.vsaPath = emmlibPath;
        }
    }
    return self;
}
